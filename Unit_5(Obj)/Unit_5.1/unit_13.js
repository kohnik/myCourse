...
document.querySelector('.b-5').addEventListener('click', () => {f5(a5, '.out-5')})

function f5(obj, domElem) {
    console.log(obj)
    let out = '';
    for(let key in obj) {
        out += `${key} : ${obj[key]} <br>`;
...
function f6() {
    let inputFirst = document.querySelector('.i-61').value;
    let inputSecond = document.querySelector('.i-62').value;

    a6[inputFirst] = inputSecond;
    f5(a6, '.out-6');
}
...
function f7() {
    ...
    let outStr = document.querySelector('.out-7');
...
function f8() {
    ...
    let outStr = document.querySelector('.out-8');

    if(a8[input] != undefined) {
        outStr.innerHTML = a8.input;
    } else {
        outStr.innerHTML = 0;
    }
}
...
//console.log(a9[key]) <<< значение
//console.log(key) <<<<< ключи
// console.log(a9['b'] = 9); <<<< меняем ключу b значение
// console.log(a9.x = 8) <<<<< создаем ключ x, присваеваем значение 8
...
function f19() {
    ...
    let out = '';
    let station = input.toLowerCase();


    for(let branch in a19) {
        for(let i = 0; i < a19[branch].length; i++) {
            if(station == a19[branch][i].toLowerCase()) {
                out += branch;
            }
        }
    }
...
function f20() {
    ...
    for(let station in a20) {
        for(let i = 0; i < a20[station].length; i++){
            if(a20[station][i][1] == 2) {
                out += a20[station][i][0] + ' ';
            }
        }
    }
