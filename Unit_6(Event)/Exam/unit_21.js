...
function t2(event) {
		clicksT2 += 1;
		document.querySelector('.out-2').innerHTML = clicksT2;
}
...
function t3(event) {
		...
		if(event.target === document.querySelector('.div-3_1')) {
				out.innerHTML = 1;
		} else {
				out.innerHTML = 2;
		}
...
function t8() {
		...
		document.querySelector('.div-8').style.backgroundColor = a8[randColor];
}
...
function t11(event) {
		...
		document.querySelector('.out-11').innerHTML = `radiusX: ${radX} radiusY: ${radY}`;
}
...
function prevFunction() {
		count - 1 >= 0 ? count -= 1 : count = images.length - 1;
		for(let item of images) {
				item.classList.remove('active-img');
		}
		images[count].classList.add('active-img');
